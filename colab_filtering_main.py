...
#PCA Components kept at 2 for consistency
PCA_COMP = 2
#the steps cluster count jumps each loop
STEP = 1
#keep gmm iterations at 500 for consistency
GMM_ITER = 500
#value directly affects Precision and Recall.
RELEVANT_TRHOLD = 4.0
#top N value for Precision and Recall calculation
TOP_N = 10 
#constant for train/test ratio, for consistency
TRAIN_TEST_SPLIT = 0.2
#neighbours value will be set to 8 for consistency
KNN_NEIGH = 8
#fraction value for data sampling, 0.7 for consistency
FRAC_SAMP = 0.7

#function that calculates prediction of unseen movie ratings
def predict_rating(user_index, movie_col,
                    user_cluster_prob, cluster_movie_means):
    ...
    for cluster_id in range(user_cluster_prob.shape[1]):
        ...
        #for each actual unseen movie rating
        ...
        if movie_col in movie_ratings and not numpy.isnan(movie_ratings[movie_col]):
            ...
            #get users probability for cluster
            prob = user_cluster_prob[user_index, cluster_id]

            #perform weighted sum probability calculation
...
#function to print the statistics for the curent cluster
def print_terminal_stats(cluster_nr,mae,rmse,precision_at,recall_at):
...
def run_gmm(train_matrix_path,test_matrix_path,sparse_test_path,run_nr,gmm_rand_state,pca_rand_state,min_n,max_n):
    ...
    # Apply PCA
    pca_transformer = PCA(n_components=PCA_COMP, 
                            random_state=pca_rand_state)
    train_matrix_pca = pca_transformer.fit_transform(
        train_matrix.values)
    test_matrix_pca = pca_transformer.transform(test_matrix.values)
    print("Fitted matrix into PCA")

    for cluster_nr in range(min_n, max_n,STEP):
        print(f"\n GMM with cluster ={cluster_nr} >")

        # Fit GMM
        gmm = GaussianMixture(n_components=cluster_nr,
                               covariance_type='full', max_iter=GMM_ITER,
                                 random_state=gmm_rand_state)
        ...
        for cluster in range(cluster_nr):
                users_in_cluster = train_matrix[
                    train_matrix["Cluster"] == cluster].drop(
                        columns=["Cluster"])
                movie_means_per_cluster[cluster] = users_in_cluster.mean(
                    skipna=True)
        ...
        #for each user in sparse matrix
        for user_index, user_id in enumerate(sparse_test_matrix.index):
            ...
            for movie_id in sparse_test_matrix.columns:
                ...
                #if unseen value is not NAN, predict rating calculation
        ...
        for user_id, rating_dict in user_predictions.items():
            ...
            if len(rating_dict) >= TOP_N:
                ...
                #sort by predicted ratings highest per user
                sorted_movies = sorted(rating_dict.items(), 
                                       key=lambda x: x[1]["pred"], reverse=True)
                top_N_movies = sorted_movies[:TOP_N]
                top_N_movie_ids = [movie_id for movie_id, _ in top_N_movies]

                #insert in the movies that pass the relevant treshold in top N
                relevant_movies = sum(1 for movie_id in top_N_movie_ids 
                                      if rating_dict[
                                          movie_id]["true"] >= RELEVANT_TRHOLD)
                
                #how many movies, beyond top N, are relevant?
                total_relevant = sum(1 for movie_data in rating_dict.values() 
                                     if movie_data[
                                         "true"] >= RELEVANT_TRHOLD)
                ...
                recall_at_k_total += (relevant_movies / total_relevant 
        ...
        mae = mean_absolute_error(y_true=true_ratings,
                                   y_pred= predicted_ratings)
        rmse = root_mean_squared_error(y_true= true_ratings,
                                       y_pred= predicted_ratings)
        ...
        print_terminal_stats(cluster_nr,mae,rmse,precision_at_k,recall_at_k)
...
#create a path to directory, for ease of use
...
def create_ds_run_gm(run_nr,sample_rand_sta,split_rand_sta,gmm_rand_sta,pca_rand_sta, max_clusters):
    ...
    pre_process.operation_data_sampling(f"{join_path(DATASET_DIR,filename_dup_rm)}.csv",sampled_file_name,FRAC_SAMP,sample_rand_sta)
    ...
    #remove low ratings from sampled dataset
    ...
    #run GMM with the dataset into BIC and AIC
    pre_process.test_bic_aic(filled_train_mat,
                           "bic_aic_results95",GMM_ITER,gmm_rand_sta,pca_rand_sta,PCA_COMP,max_clusters,run_nr)
